...
**Next:** Riparian (NBS capstone, 268k features — needs heavy dissolve), optionally HEC-RAS depth raster, USFWS wetlands. Then the workshop narrative scaffolding (how layers sequence into the 2027 scale-up argument). Pipeline = fetch (ArcGIS f=geojson, paged) → mapshaper simplify/clip to wv_outline → data/.
